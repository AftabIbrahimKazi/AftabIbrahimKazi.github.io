...
**Date:** 2026-06-21
**Last updated:** 2026-07-08 — Phase 19: `TextureBump` retired (last custom raw-GLSL node in `CoreShaderNodes.ts` — shader-core 0.4.1→0.4.2 fixed the upstream `Bump({method:'uv-offset'})` extension-ordering bug); warp veil now covers the page from first paint via CSS default state instead of a JS-driven flash on load/back-navigation; landing page's journal Mercury orb gained crater bump relief. **Uncommitted on `dev`** — not pushed.
