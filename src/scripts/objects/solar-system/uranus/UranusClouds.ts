// src/scripts/objects/solar-system/uranus/UranusClouds.ts
...
/**
 * Uranus cloud layer — triforge node-graph port of the original
 * MeshStandardMaterial + onBeforeCompile GLSL:
 *
 *   terminator = smoothstep(-0.4, 0.4, dot(vWorldNormal, sunDir))
 *   rgb        = cloudTex * terminator
 *   alpha      = 0.3                       // constant opacity, additive blending
 *
 * Same 2k_uranus texture as the surface, panned independently (UvPan) to
 ...
 */
export function buildUranusCloudMaterial(cloudTexture: THREE.Texture): PannableMaterial {
  ...
  const blended = new MixShader({ fac: 0.3, shader1: new TransparentBSDF().output('BSDF'), shader2: clouds.output('BSDF') });
