...
/**
 * Neptune surface — triforge node-graph port of the original
 ...
 * (sun at scene origin). The deep-blue emissive (0x1a3a9a × 0.25) of the
...
export function buildNeptuneSurfaceMaterial(neptuneTexture: THREE.Texture): PannableMaterial {
  ...
  const nightTint  = new Emission({ color: [0.01, 0.01, 0.04] as unknown as string, strength: nightBlend.output('Value') });
