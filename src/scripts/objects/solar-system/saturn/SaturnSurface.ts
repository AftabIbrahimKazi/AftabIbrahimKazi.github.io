...
/**
 * Saturn surface — triforge node-graph port of the original
 ...
 * (sun at scene origin). The band emissive (0xc8b080 × 0.025) of the original
...
export function buildSaturnSurfaceMaterial(saturnTexture: THREE.Texture): PannableMaterial {
  ...
  const surfaceTex = new ImageTexture({ uniformName: 'uSaturnTex', vector: uvPan.output('UV') });
  ...
  const nightTint  = new Emission({ color: [0.04, 0.03, 0.01] as unknown as string, strength: nightBlend.output('Value') });
