// src/scripts/objects/solar-system/mars/MarsAtmosphere.ts
...
/**
 * Mars atmosphere — triforge node-graph port of the original onBeforeCompile GLSL:
 *
 *   terminator = smoothstep(0.0, 0.4, dot(vWorldNormal, sunDir))
 *   fresnel    = pow(1.0 - dot(N, V), 6.0)
 *   rgb        = vec3(0.75, 0.35, 0.15) * fresnel * terminator
 ...
 */
export function buildMarsAtmosphereMaterial(): THREE.ShaderMaterial {
  ...
  const glow      = new Emission({ color: [0.75, 0.35, 0.15] as unknown as string, strength: intensity.output('Value') });
