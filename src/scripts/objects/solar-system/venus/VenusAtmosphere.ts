// src/scripts/objects/solar-system/venus/VenusAtmosphere.ts
...
/**
 * Venus atmosphere — triforge node-graph port of the original onBeforeCompile GLSL:
 *
 *   terminator = smoothstep(0.0, 0.4, dot(vWorldNormal, sunDir))
 *   fresnel    = pow(1.0 - dot(N, V), 6.0)
 *   rgb        = vec3(1.0, 0.85, 0.6) * fresnel * terminator
 ...
 */
export function buildVenusAtmosphereMaterial(): THREE.ShaderMaterial {
  ...
  const glow      = new Emission({ color: [1.0, 0.85, 0.6] as unknown as string, strength: intensity.output('Value') });
