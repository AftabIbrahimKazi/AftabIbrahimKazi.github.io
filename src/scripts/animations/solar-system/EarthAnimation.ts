...
export class EarthAnimation {
  ...
  update(delta: number): void {
    if (!this.earth.mesh || !this.earth.pivot) return;
  ...
  private _updateSunDirection(): void {
    const material = this.earth.mesh.material as THREE.ShaderMaterial;
    if (!material.uniforms || !material.uniforms['uSunDirection']) return;

    this.earth.mesh.getWorldPosition(this._worldPos);
  ...
  private _init(): AnimationMixer | null {
    const { mesh, pivot, cloud } = this.earth;
    ...
    mixer.play(new AnimationClip('orbit', [
      new KeyframeTrack(pivotRot, 'y', [{ time: 0, value: 0 }, { time: 1200, value: Math.PI * 2 }]),
    ]), { wrapMode: 'loop' });

    const meshRot = mesh.rotation as unknown as Record<string, number>;
    mixer.play(new AnimationClip('self-rot', [
      new KeyframeTrack(meshRot, 'y', [{ time: 0, value: 0 }, { time: 120, value: Math.PI * 2 }]),
    ]), { wrapMode: 'loop' });

    if (cloud) {
      const cloudRot = cloud.rotation as unknown as Record<string, number>;
      mixer.play(new AnimationClip('cloud-rot', [
        new KeyframeTrack(cloudRot, 'y', [{ time: 0, value: 0 }, { time: 240, value: Math.PI * 2 }]),
      ]), { wrapMode: 'loop' });
    }

    const glow = mesh.getObjectByName('ex-earth-glow-js') as THREE.Sprite | undefined;
    if (glow) {
      const glowScale = glow.scale as unknown as Record<string, number>;
      mixer.play(new AnimationClip('glow', [
        new KeyframeTrack(glowScale, 'x', [{ time: 0, value: 3.0, easing: easeInOutSine }, { time: 3, value: 3.2 }]),
        new KeyframeTrack(glowScale, 'y', [{ time: 0, value: 3.0, easing: easeInOutSine }, { time: 3, value: 3.2 }]),
      ]), { wrapMode: 'pingpong' });
    }
