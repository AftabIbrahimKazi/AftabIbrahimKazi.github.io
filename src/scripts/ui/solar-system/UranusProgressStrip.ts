// src/scripts/ui/solar-system/UranusProgressStrip.ts
// Uranus-only left progress strip — same dual-role pattern as the other
// planet progress strips (see MercuryProgressStrip/VenusProgressStrip for
// the full rationale). Scoped entirely to #ex-uranus-progress-strip /
// .ex-uranus-section / #ex-uranus-article-panel-body.

interface ItemLabel { number: string; label: string; }

const LABEL_SWAP_MS = 160;

export class UranusProgressStrip {
  ...
  constructor() {
    this.strip      = document.getElementById('ex-uranus-progress-strip');
    this.items      = Array.from(document.querySelectorAll<HTMLElement>('.ex-uranus-progress-item'));
    this.sections   = Array.from(document.querySelectorAll<HTMLElement>('.ex-uranus-section'));
    this.overlay    = document.getElementById('ex-content-overlay');
    this.panelInner = document.querySelector<HTMLElement>('.ex-uranus-article-panel-inner');
    this.panelBody  = document.getElementById('ex-uranus-article-panel-body');
  }
  ...
  init(): void {
    ...
    this.originalLabels = this.items.map((item) => ({
      number: item.querySelector('.ex-uranus-progress-number')?.textContent ?? '',
      label:  item.querySelector('.ex-uranus-progress-label')?.textContent  ?? '',
    }));
  ...
  private _bindPanelEvents(): void {
    window.addEventListener('ct:uranus-article-open',  () => this._enterArticleMode());
    window.addEventListener('ct:uranus-article-close', () => this._exitArticleMode());
  }
  ...
  private _swapLabels(next: (item: HTMLElement, i: number) => { number: string; label: string; inRange: boolean }): void {
    this.items.forEach((item, i) => {
      ...
      window.setTimeout(() => {
        const numberEl = item.querySelector<HTMLElement>('.ex-uranus-progress-number');
        const labelEl  = item.querySelector<HTMLElement>('.ex-uranus-progress-label');
