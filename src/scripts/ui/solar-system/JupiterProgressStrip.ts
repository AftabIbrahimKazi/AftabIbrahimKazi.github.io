// src/scripts/ui/solar-system/JupiterProgressStrip.ts
// Jupiter-only left progress strip — same dual-role pattern as the other
// planet progress strips (see MercuryProgressStrip/VenusProgressStrip for
// the full rationale). Scoped entirely to #ex-jupiter-progress-strip /
// .ex-jupiter-section / #ex-jupiter-article-panel-body.

interface ItemLabel { number: string; label: string; }

const LABEL_SWAP_MS = 160;

export class JupiterProgressStrip {
  ...
  constructor() {
    this.strip      = document.getElementById('ex-jupiter-progress-strip');
    this.items      = Array.from(document.querySelectorAll<HTMLElement>('.ex-jupiter-progress-item'));
    this.sections   = Array.from(document.querySelectorAll<HTMLElement>('.ex-jupiter-section'));
    this.overlay    = document.getElementById('ex-content-overlay');
    this.panelInner = document.querySelector<HTMLElement>('.ex-jupiter-article-panel-inner');
    this.panelBody  = document.getElementById('ex-jupiter-article-panel-body');
  }
  ...
  init(): void {
    ...
    this.originalLabels = this.items.map((item) => ({
      number: item.querySelector('.ex-jupiter-progress-number')?.textContent ?? '',
      label:  item.querySelector('.ex-jupiter-progress-label')?.textContent  ?? '',
    }));
  ...
  private _bindPanelEvents(): void {
    window.addEventListener('ct:jupiter-article-open',  () => this._enterArticleMode());
    window.addEventListener('ct:jupiter-article-close', () => this._exitArticleMode());
  }
  ...
  private _swapLabels(next: (item: HTMLElement, i: number) => { number: string; label: string; inRange: boolean }): void {
    this.items.forEach((item, i) => {
      ...
      window.setTimeout(() => {
        const numberEl = item.querySelector<HTMLElement>('.ex-jupiter-progress-number');
        const labelEl  = item.querySelector<HTMLElement>('.ex-jupiter-progress-label');
