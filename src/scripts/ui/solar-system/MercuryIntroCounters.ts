// src/scripts/ui/solar-system/MercuryIntroCounters.ts
// Counts each `.ex-mercury-counter` up from 0 to its `data-count-to` value
// once it scrolls into view inside the Mercury overlay. Scoped to the
...
export class MercuryIntroCounters {

  private counters: HTMLElement[];
  private root:     HTMLElement | null;

  constructor() {
    this.counters = Array.from(document.querySelectorAll<HTMLElement>('.ex-mercury-counter'));
