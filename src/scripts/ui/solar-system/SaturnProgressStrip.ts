// src/scripts/ui/solar-system/SaturnProgressStrip.ts
// Saturn-only left progress strip — same dual-role pattern as the other
// planet progress strips (see MercuryProgressStrip/VenusProgressStrip for
// the full rationale). Scoped entirely to #ex-saturn-progress-strip /
// .ex-saturn-section / #ex-saturn-article-panel-body.

interface ItemLabel { number: string; label: string; }

const LABEL_SWAP_MS = 160;

export class SaturnProgressStrip {
  ...
  constructor() {
    this.strip      = document.getElementById('ex-saturn-progress-strip');
    this.items      = Array.from(document.querySelectorAll<HTMLElement>('.ex-saturn-progress-item'));
    this.sections   = Array.from(document.querySelectorAll<HTMLElement>('.ex-saturn-section'));
    this.overlay    = document.getElementById('ex-content-overlay');
    this.panelInner = document.querySelector<HTMLElement>('.ex-saturn-article-panel-inner');
    this.panelBody  = document.getElementById('ex-saturn-article-panel-body');
  }
  ...
  init(): void {
    ...
    this.originalLabels = this.items.map((item) => ({
      number: item.querySelector('.ex-saturn-progress-number')?.textContent ?? '',
      label:  item.querySelector('.ex-saturn-progress-label')?.textContent  ?? '',
    }));
  ...
  private _bindPanelEvents(): void {
    window.addEventListener('ct:saturn-article-open',  () => this._enterArticleMode());
    window.addEventListener('ct:saturn-article-close', () => this._exitArticleMode());
  }
  ...
  private _swapLabels(next: (item: HTMLElement, i: number) => { number: string; label: string; inRange: boolean }): void {
    this.items.forEach((item, i) => {
      ...
      window.setTimeout(() => {
        const numberEl = item.querySelector<HTMLElement>('.ex-saturn-progress-number');
        const labelEl  = item.querySelector<HTMLElement>('.ex-saturn-progress-label');
